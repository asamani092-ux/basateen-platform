#!/usr/bin/env node
/**
 * 054 remote: tasks_snapshot on edu_daily_recitation + competition_logs.
 *
 * Usage (from apps/api):
...
if (isWinArm64) {
  console.error(`
❌ Windows ARM64: use GitHub Actions (docs/PRODUCTION.md) for db:remote:054.
`);
  process.exit(1);
}
...
run(
  `--file="${path.join(schemaDir, "054_tasks_snapshot.sql")}"`,
  "054 tasks_snapshot columns",
  { allowFail: true },
);
