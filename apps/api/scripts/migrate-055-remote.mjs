#!/usr/bin/env node
/**
 * 055 remote: metrics_json + source on competition_logs for day achievement.
 *
 * Usage (from apps/api):
...
if (isWinArm64) {
  console.error(`
❌ Windows ARM64: use GitHub Actions (docs/PRODUCTION.md) for db:remote:055.
`);
  process.exit(1);
}
...
run(
  `--file="${path.join(schemaDir, "055_competition_logs_metrics.sql")}"`,
  "055 competition_logs metrics_json",
  { allowFail: true },
);
