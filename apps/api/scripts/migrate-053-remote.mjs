#!/usr/bin/env node
/**
 * 053 remote: criterion_id on competition_tasks.
 *
 * Usage (from apps/api):
...
if (isWinArm64) {
  console.error(`
❌ Windows ARM64: use GitHub Actions (docs/PRODUCTION.md) for db:remote:053.
`);
  process.exit(1);
}
...
run(
  `--file="${path.join(schemaDir, "053_competition_criterion_id.sql")}"`,
  "053 competition criterion_id",
  { allowFail: true },
);
